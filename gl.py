...
class Renderer(object):
    ...
    # Obtener color de pixeles
    def getFramebuffer(this, x, y):
    ...
    # Check point inside polygon https://stackoverflow.com/questions/36399381/whats-the-fastest-way-of-checking-if-a-point-is-inside-a-polygon-in-python
    def glCheck(this, polygon, x, y):
        vertex = len(polygon)
        i = 0
        j = vertex - 1
        inIt = False
        for i in range(vertex):
            if ((polygon[i][1] > y) != (polygon[j][1] > y)) and (x < polygon[i][0] + (polygon[j][0] - polygon[i][0]) * (y - polygon[i][1]) / (polygon[j][1] - polygon[i][1])):
                inIt = not inIt

            j = i
        return inIt

    def glPointLine(this, x, y):
        this.framebuffer[y][x] = this.currColor

    # Algoritmo de Bresenham para creación de lineas
    def glLine(this, x1, y1, x2, y2, color=None):
        ...
        for x in range(x1, x2):
            if steep:
                this.glPointLine(y, x)
            else:
                this.glPointLine(x, y)
    ...
    # Dibujar figura
    def glDraw(this, vert):
        n = len(vert)
        for i in range(n):
            v0 = vert[i]
            v1 = vert[(i+1) % n]
            this.glLine(v0[0], v0[1], v1[0], v1[1])

    def glFill(this, polygon):
        vertexX = []
        vertexY = []
        vertex = []
        n = len(polygon)
